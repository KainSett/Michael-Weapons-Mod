...
namespace MichaelWeaponsMod.Content
{
    ...
    public class DictUpdater : GlobalNPC
    {
        ...
        public void EmitLight(Vector2 position, float power)
        {
            ...
            // Calculate the center of the NPC in world coordinates
            Vector2 center = position + Main.screenPosition;

            // Calculate the light radius in tiles (1 tile = 16 world units)
            int radiusInTiles = (int)power;

            // Get the center tile coordinates
            int centerX = (int)(center.X / 16);
            int centerY = (int)(center.Y / 16);

            // Loop over a square area and check each tile if it's within the circular radius
            for (int x = centerX - radiusInTiles; x <= centerX + radiusInTiles; x++)
            {
                for (int y = centerY - radiusInTiles; y <= centerY + radiusInTiles; y++)
                {
                    // Calculate the distance from the center tile
                    if (Vector2.Distance(new Vector2(x, y), new Vector2(centerX, centerY)) <= radiusInTiles)
                    {
                        // Apply the lighting effect on the tile
                        Lighting.GetColor(new Point(x, y), new Color(r * 3, g * 3, b * 3)); // Adjust RGB values for color and intensity
                    }
                }
            }
        }
